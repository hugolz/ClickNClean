		...
		if (name.isEmpty() || surname.isEmpty() || email.isEmpty() || password.isEmpty() || confirmpassword.isEmpty() || phone.isEmpty() || birthDate==null || address==null || km==0 || hourlyRate.isEmpty() || biography.isEmpty() || motivation.isEmpty() || experience.isEmpty() || photo.isEmpty() || idPhoto.isEmpty()) {
			...
			if (password==confirmpassword && phone.length()==10) {
				...
				db.DAOAddCleaner(name, password, surname, email, phone, birthDate, false, address, km, hourlyRate, biography, idPhoto, motivation, experience, false, photo, photoLive);
							//(String name, String pwd, String surname, String email, String phoneN, LocalDate birthDate, boolean isSuspended, UserStatus status, Address departureAddress, int kmRange, int hourlyRate, String bio, String photo, String motivation, String experience, boolean isConfirmed, String photoProfile, String photoLive) 
			}
			else {
